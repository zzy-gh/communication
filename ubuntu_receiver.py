...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run a small HTTP server to receive text snippets."
    )
    parser.add_argument("--host", default="0.0.0.0", help="Bind host, default: 0.0.0.0")
    parser.add_argument("--port", type=int, default=8765, help="Bind port, default: 8765")
    parser.add_argument("--name", default="receiver", help="Friendly device name")
    parser.add_argument(
        "--output",
        default="received_messages.txt",
        help="File used to store received text",
    )
    parser.add_argument(
        "--token",
        default="",
...
def build_handler(output_file: Path, token: str, device_name: str):
    class ReceiverHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            self._send_json(
                ...
                {
                    "ok": True,
                    "message": f"{device_name} receiver is running",
...
def main():
    ...
    handler_cls = build_handler(output_file, args.token, args.name)
    server = HTTPServer((args.host, args.port), handler_cls)

    print(f"{args.name} listening on http://{args.host}:{args.port}")
    print(f"Messages will be saved to: {output_file.resolve()}")
    print("Token check: enabled" if args.token else "Token check: disabled")
